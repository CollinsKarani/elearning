                ...
                <div class="span9" id="content">
                     <div class="row-fluid">
					    <!-- breadcrumb -->	
									<ul class="breadcrumb">
										<?php
										$school_year_query = $conn->query("select * from school_year order by school_year DESC")or die(mysql_error());
                        ...
                        <div id="block_bg" class="block">
                            ...
                            <div class="block-content collapse in">
							    ...
							    <form class="form-horizontal" method="post">
										...
										<div class="control-group">
											...
											<div class="controls">			
												<select id="qtype" name="question_tpye" required>
													...
													<?php
													$query_question = $conn->query("select * from question_type")or die(mysql_error());
										...
										<div id="opt11">
<?php
	$sqlz = $conn->query("SELECT * FROM quiz_question WHERE quiz_question_id ='$quiz_question_id'");
...
<div id="opt12">
	...
	<input name="correctt" <?php if($row['answer'] == 'False'){ echo 'checked'; }?> value="f" type="radio">False<br /><br />
		...
		if ($type  == '2'){
				$conn->query("insert into quiz_question (quiz_id,question_text,date_added,answer,question_type_id)
		...
		$query = $conn->query("select * from quiz_question order by quiz_question_id DESC LIMIT 1")or die(mysqli_error($conn));
		$row = mysqli_fetch_array($query);
		$quiz_question_id = $row['quiz_question_id'];

		$conn->query("insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans1','A')")or die(mysql_error());
		$conn->query("insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans2','B')")or die(mysql_error());
		$conn->query("insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans3','C')")or die(mysql_error());
		$conn->query("insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans4','D')")or die(mysql_error());
