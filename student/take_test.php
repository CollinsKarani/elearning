...
<?php $query1 = $conn->query("select * from student_class_quiz where student_id = '$session_id' and class_quiz_id = '$class_quiz_id' ")or die(mysql_error());
...
$conn->query("insert into student_class_quiz (class_quiz_id,student_id,student_quiz_time) values('$class_quiz_id','$session_id','$quiz_time')");
}
 ?>
...
$sqlp = $conn->query("SELECT * FROM class_quiz WHERE class_quiz_id = '$class_quiz_id'")or die(mysql_error());
...
<?php
										$sqla = $conn->query("select * FROM class_quiz
...
<?php
	$sqlw = $conn->query("SELECT * FROM quiz_question where quiz_id = '$quiz_id'  ORDER BY RAND()");
...
<?php
	...
	$sqly = $conn->query("SELECT * FROM answer WHERE quiz_question_id = '".$roww['quiz_question_id']."'");
...
<?php
	...
	for($x=1;$x<=$x1;$x++){
		...
		$sql =$conn->query("SELECT * FROM quiz_question WHERE quiz_question_id = ".$x2."");
   ...
   $conn->query("UPDATE student_class_quiz SET `student_quiz_time` = 3600, `grade` = '".$score." out of ".($x-1)."' WHERE student_id = '$session_id' and class_quiz_id = '$class_quiz_id'")or die(mysql_error());
?>
<script>
